...
module.exports = (
    ...
    const cleanByLimitHandler = toAsync(async (req, res, next) => {
        ...
        if (toBeCleaned !== false) {
            logger.log('Maximum caches is exceeds, you can clean:', toBeCleaned)
    ...
    return express()
         ...
         *
         */
        .use(toAsync(async (req, res, next) => {
            const count = await cacheManager.cleanTTL(config.expirationTime);
            if (count > 0) {
         ...
         *     cache.data  = null
         * Or clean up expire time is over than 2/3 of time
         *
         * If cache limit is over
